...
| Admin-Login antwortet mit `204`, aber die nächste Anfrage (`/api/admin/me`) liefert trotzdem `401`, mit **neuem** `JSESSIONID` | Cloud Run hat mehrere Instanzen, Spring hält Sitzungen nur im Arbeitsspeicher der jeweiligen Instanz — die Folgeanfrage landet auf einer Instanz, die das Cookie nicht kennt. `--session-affinity` allein behebt das **nicht zuverlässig**, weil Firebase Hosting sein Routing-Cookie beim Weiterleiten an Cloud Run nicht sauber durchreicht (verifiziert: direkt gegen die Cloud-Run-Adresse funktioniert Affinity, über Firebase Hosting nicht). [`deploy-cloudrun.yml`](../.github/workflows/deploy-cloudrun.yml) setzt deshalb zusätzlich `--max-instances=1` — bei der Größe dieser App unproblematisch. Bei ältere Deploys einmal neu ausrollen. Robustere, aufwändigere Alternative bei mehr Traffic: ein geteilter Sitzungsspeicher (z. B. Spring Session mit der Neon-Datenbank) statt der Instanzbegrenzung |
| Admin-Login antwortet direkt mit `401` (nicht erst die Folgeanfrage) | `SERVER_FORWARD_HEADERS_STRATEGY=native` fehlt unter den `env_vars` beim Cloud-Run-Deploy-Schritt, oder Zugangsdaten falsch — im Browser via Entwicklertools → Network → Payload der `/api/admin/login`-Anfrage prüfen |
