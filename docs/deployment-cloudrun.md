...
| Admin-Login antwortet mit `204`, aber die nächste Anfrage (`/api/admin/me`) liefert trotzdem `401`, mit **neuem** `JSESSIONID` | Cloud Run hat mehrere Instanzen, Spring hält Sitzungen nur im Arbeitsspeicher der jeweiligen Instanz — die Folgeanfrage landet auf einer Instanz, die das Cookie nicht kennt. Braucht `--session-affinity` in den `flags` des Deploy-Schritts (seit [`deploy-cloudrun.yml`](../.github/workflows/deploy-cloudrun.yml) gesetzt; bei älteren Deploys einmal neu ausrollen). Best effort, keine Garantie — bei anhaltenden Problemen wäre ein geteilter Sitzungsspeicher (z. B. Spring Session mit der Neon-Datenbank) die robustere, aufwändigere Lösung |
